...
class Config:
    ...
    # down the training.
    VALIDATION_STEPS = 100          #注意这里从50改成了100
    ...
    # Bayesian location regression
    # If True, location head predicts both mean and log-variance.
    BAYESIAN_LOC = False
    #MC_Dropout开关
    MC_DROPOUT = False
    DROPOUT_RATE = 0.2
    ...
    # Numerical stability / clipping for Bayesian location regression
    LOC_LOGVAR_MIN = -4.0
    LOC_LOGVAR_MAX = 4.0
    LOC_MAX_PER_DIM_LOSS = 50.0
    LOC_EPS = 1e-6
    ...
    # Soft assignment parameter to scale gaussian stddev
    BETA = 6.0
    ...
    # Sets Keras datatype to 16 bit instead of 32 bit
    # (only works reliably with supported hardware/software stack)
    F16 = True
    ...
    def update(self):
        ...
        if self.BAYESIAN_LOC:
            assert self.REGRESS_LOC, "BAYESIAN_LOC requires REGRESS_LOC=True"
            assert not self.REGRESS_KEYPOINTS, \
                "BAYESIAN_LOC is not supported with REGRESS_KEYPOINTS"

        if self.ORIENTATION_PARAM not in ['quaternion', 'euler_angles', 'angle_axis']:
            raise ValueError(
                "ORIENTATION_PARAM must be one of "
                "['quaternion', 'euler_angles', 'angle_axis']"
            )

        if self.LOC_LOGVAR_MIN > self.LOC_LOGVAR_MAX:
            raise ValueError("LOC_LOGVAR_MIN must be <= LOC_LOGVAR_MAX")
